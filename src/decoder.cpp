  ...
  void InitFFmpegDecoder() {
    ...
    if (hw_type_ != AV_HWDEVICE_TYPE_NONE) {
      int err = av_hwdevice_ctx_create(&hw_device_ctx_, hw_type_, nullptr,
                                       nullptr, 0);
      if (err < 0) {
        RCLCPP_WARN(this->get_logger(), "Failed to create hardware device "
                                        "context, falling back to software");
        hw_type_ = AV_HWDEVICE_TYPE_NONE;
        codec_ = (AVCodec *)avcodec_find_decoder(AV_CODEC_ID_H264);
        if (!codec_) {
          RCLCPP_ERROR(this->get_logger(), "No H.264 decoder available");
    ...
    while (ret >= 0) {
      ...
      if (!sws_ctx_ && frame_to_display->width > 0 &&
        ...
        sws_ctx_ = sws_getContext(
            ...
            (AVPixelFormat)frame_to_display->format, frame_to_display->width,
            frame_to_display->height, AV_PIX_FMT_BGR24, SWS_POINT, nullptr,
            nullptr, nullptr);
        ...
        bgr_frame_.create(frame_to_display->height, frame_to_display->width,
                          CV_8UC3);
      }
      ...
      if (sws_ctx_ && !bgr_frame_.empty()) {
        ...
        sws_scale(sws_ctx_, (const uint8_t *const *)frame_to_display->data,
                  frame_to_display->linesize, 0, frame_to_display->height,
  ...
  void compressed_image_callback(
    ...
    while (remaining_size > 0) {
      int bytes_parsed = av_parser_parse2(
          ...
          static_cast<int>(remaining_size), AV_NOPTS_VALUE, AV_NOPTS_VALUE, 0);
      if (bytes_parsed < 0) {
        break;
      }
...
public:
  H264DecoderNode() : Node("h264_decoder_node") {
    this->declare_parameter("compressed_topic",
    ...
    std::string subscribe_topic =
        this->get_parameter("compressed_topic").as_string();
    std::string publish_topic =
        this->get_parameter("uncompressed_topic").as_string();
    ...
    subscription_ =
        this->create_subscription<sensor_msgs::msg::CompressedImage>(
            subscribe_topic, 10,
            std::bind(&H264DecoderNode::compressed_image_callback, this,
                      std::placeholders::_1));

    publisher_ =
        this->create_publisher<sensor_msgs::msg::Image>(publish_topic, 10);
    ...
    RCLCPP_INFO(this->get_logger(), "Skip frame: %d, I-frame only: %s",
                skip_frame_, i_frame_only_ ? "true" : "false");
  }
